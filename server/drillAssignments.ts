...
/**
 * Get weekly goals for an athlete
 */
export async function getWeeklyGoals(athleteId: number) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
  ...
  const goals = await db
    .select()
    .from(weeklyGoals)
    .where(eq(weeklyGoals.athleteId, athleteId))
...
/**
 * Create a new weekly goal for an athlete
 */
export async function createWeeklyGoal(data: {
  ...
  const goalData = {
    ...
    targetDrillCount: data.targetDrillCount,
...
export async function deleteWeeklyGoal(goalId: number) {
  ...
  await db.delete(weeklyGoals).where(eq(weeklyGoals.id, goalId));
  return { success: true };
}

/**
 * Get current week's goal for an athlete
 */
export async function getCurrentWeekGoal(athleteId: number) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
  ...
  const goal = await db
    ...
    .where(
      and(
        eq(weeklyGoals.athleteId, athleteId),
        lte(weeklyGoals.weekStartDate, now),
        gte(weeklyGoals.weekEndDate, now)
      )
    )
    .limit(1);
...
/**
 * Calculate the current streak of consecutive days with drill activity
...
export async function calculateStreak(userId: number): Promise<number> {
  ...
  // Get all completed drills ordered by completion date descending
  const completedDrills = await db
    .select({
      completedAt: drillAssignments.completedAt,
    })
    .from(drillAssignments)
    .where(
      and(
        ...
        sql`${drillAssignments.completedAt} IS NOT NULL`
      )
    )
    .orderBy(desc(drillAssignments.completedAt));
  ...
  completedDrills.forEach((drill) => {
    if (drill.completedAt) {
      const dateStr = new Date(drill.completedAt).toISOString().split('T')[0];
